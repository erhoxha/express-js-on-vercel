...
export function calculateLotSize({
    ...
    stopLossPrice,
    accountBalance,
    riskPercentageUWantToRisk,
    goldOrJPYPair
}: CalculateLotSizeParams): CalculateLotSizeResult {
    const entry = parseFloat(String(entryPrice));
    const stop = parseFloat(String(stopLossPrice));
    const balance = parseFloat(String(accountBalance));
    const riskPct = parseFloat(String(riskPercentageUWantToRisk));
 ...
 */
function calculatePipDifference(entry: number, stop: number, isGoldOrJPY: boolean): number {
    if (isGoldOrJPY) {
 ...
 */
function roundToBrokerLot(value: number): number {
